...
export default {
  ...
  methods: {
    ...
    handleNodeClick (data) {
      ...
      this.$get('/admin/department/info/' + data.id).then(res => {
        if (res.code === 1) {
    ...
    getDepartmentTree () {
      this.$get('/admin/department/tree').then(res => {
        if (res.code === 1) {
    ...
    deleteNode (data) {
      this.$confirm('是否确定要删除?', '提示', {
        ...
        this.$get('/admin/department/delete/' + data.id).then(res => {
          if (res.code === 1) {
            this.getDepartmentTree()
    ...
    saveForm () {
      this.$refs.form.validate((valid, object) => {
        if (!valid) {
          ...
          this.$post('/admin/department/save', this.formData).then(res => {
            if (res.code === 1) {
              this.$message.success(res.msg)
              this.getDepartmentTree()
              this.clearForm()
